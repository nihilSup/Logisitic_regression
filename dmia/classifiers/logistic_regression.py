...
class LogisticRegression:
    ...
    def train(self, X, y, learning_rate=1e-3, reg=1e-5, num_iters=100,
        ...
        if self.w is None:
            ...
            # self.w = np.random.randn(dim) * 0.0001
            self.w = np.zeros(dim)
        ...
        for it in range(num_iters):
            ...
            #########################################################################
            self.update_weights(learning_rate, gradW)
    ...
    def loss(self, X_batch, y_batch, reg):
        ...
        """
        num_train = y_batch.shape[0]
        dw = np.zeros_like(self.w)  # initialize the gradient as zero
        # Compute loss and gradient. Your code should not contain python loops.
        h = self.sigmoid(X_batch)
        loss = (-y_batch * np.log(h) - (1.0 - y_batch) * np.log(1.0 - h)).mean()
        dw = X_batch.T @ (h - y_batch) / num_train
        ...
        loss += (reg/(2 * num_train)) * np.sum(self.w[:-1]**2)
